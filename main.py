# Set up and run this Streamlit App
import streamlit as st
# from helper_functions import llm # <--- This is the helper function that we have created 🆕
from logics import customer_query_handler as cqh
import pandas as pd
from helper_functions.utility import check_password  

# region <--------- Streamlit App Configuration --------->
st.set_page_config(
    layout="centered",
    page_title="My Streamlit App"
)
# endregion <--------- Streamlit App Configuration --------->

# Check if the password is correct.  
if not check_password():  
    st.stop()

st.title("Streamlit App")

form = st.form(key="form")
form.subheader("Identify Category and Courses")

user_prompt = form.text_area("Enter your prompt here", height=200)

if form.form_submit_button("Submit"):
    st.toast(f"User Input Submitted - {user_prompt}")
    response, course_details = cqh.process_user_message(user_prompt) # <--- This calls the helper function that we have created 🆕
    st.divider()
    st.write(response) # <--- This displays the response generated by the LLM onto the frontend 🆕
    print(f"User Input is {user_prompt}")
        
    df = pd.DataFrame(course_details)
    st.divider()
    st.dataframe(df)